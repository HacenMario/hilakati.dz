...
// النماذج (تأكد من وجودها في مجلد models)
...
app.post('/api/auth/forgot-password', async (req, res) => {
  ...
  try {
    let user;
    if (userType === 'salon') user = await Salon.findOne({ email });
    else if (userType === 'customer') user = await Customer.findOne({ email });
    else return res.status(400).json({ message: 'نوع المستخدم غير صحيح' });
    ...
    const resetUrl = `${process.env.FRONTEND_URL || 'http://localhost:5000'}?token=${resetToken}&userType=${userType}`;
    ...
    await transporter.sendMail({
      ...
      html: `
        <div dir="rtl" style="font-family: 'Tajawal', sans-serif; text-align: right; background: #f8f9fa; padding: 20px; border-radius: 10px;">
          ...
          <p>أو يمكنك نسخ هذا الرابط ولصقه في متصفحك:</p>
          <p style="background: #ffffff; padding: 10px; border-radius: 5px; direction: ltr; text-align: left;">${resetUrl}</p>
